...
def test_splits_functions_in_a_file(tmp_path):
    file_path = str(tmp_path / "file1.ts")
    file_contents = """
function testfunc() {
    console.log("nothing");
}
...
""".lstrip()
    ...
    splitter = split_code.TypescriptSplitter(tmp_path, parser_path)
...
def test_splits_es6_func_with_multiple_variables(tmp_path):
    file_path = str(tmp_path / "file1.ts")
    file_contents = """
const somefunc = (a: number, b: float, c: string) => {
    console.log(a, b, c);
}
"""
...
@pytest.mark.parametrize("file_path", ("test.ts", "test.js", "some/path/to/file.ts"))
